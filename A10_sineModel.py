...
def sineModel_Multi(x, fs, w1, w2, w3, N1, N2, N3, t, B1, B2, B3):
    """
    Week 10, Project:  A multi-resolution sinusoidal model
    Analysis/synthesis of a sound using the sinusoidal model, without sine tracking
    Using Multi-resolution sine model
    ...
    while pin < pend:  # while input sound pointer is within sound
        ...
        # Getting peaks which fall in selected frequency bands
        ipfreq = ipfreq1[indice_1]
        ipmag = ipmag1[indice_1]
        ipphase = ipphase1[indice_1]
    ...
    while pin < pend:  # while input sound pointer is within sound
        ...
        ploc2 = UF.peakDetection(mX2, t)  # detect locations of peaks for 2nd frame
        ploc3 = UF.peakDetection(mX3, t)  # detect locations of peaks for 3rd frame
        iploc1, ipmag1, ipphase1 = UF.peakInterp(mX1, pX1,
                                                 ploc1)  # refine peak values by interpolation for the 1st frame
        iploc2, ipmag2, ipphase2 = UF.peakInterp(mX2, pX2,
                                                 ploc2)  # refine peak values by interpolation for the 2nd frame
        iploc3, ipmag3, ipphase3 = UF.peakInterp(mX3, pX3,
        ...
        # Getting peaks which fall in selected frequency bands
        ipfreq = ipfreq2[indice_2]
        ipmag = ipmag2[indice_2]
        ipphase = ipphase2[indice_2]
    ...
    while pin < pend:  # while input sound pointer is within sound
        ...
        iploc3, ipmag3, ipphase3 = UF.peakInterp(mX3, pX3,
                                                 ploc3)  # refine peak values by interpolation for the 3rd frame
        ipfreq1 = fs * iploc1 / float(N1)  # convert peak locations of 1st frame to Hertz for the 1st window
        ipfreq2 = fs * iploc2 / float(N2)  # convert peak locations of 2nd frame to Hertz for the 2nd window
        ipfreq3 = fs * iploc3 / float(N3)  # convert peak locations of 3rd frame to Hertz for the 3rd window
        ...
        # Getting peaks which fall in selected frequency bands
        ipfreq = ipfreq3[indice_3]
        ipmag = ipmag3[indice_3]
        ipphase = ipphase3[indice_3]
...
fs, x = UF.wavread('331025__dshoot85__orchestral-loop-with-oriental-touch-mono.wav')
...
UF.wavwrite(y, fs, 'sin_orchestra.wav')